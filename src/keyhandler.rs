...
impl KeyCombinationTracker {
    ...
    pub fn check_combinations(&mut self) -> KeyCombination {
        ...
        // Check for Ctrl key
        let is_ctrl_pressed = keys.contains(&Keycode::LControl) || keys.contains(&Keycode::RControl);
...
pub fn handle_key_combination(app: &mut App, combination: KeyCombination) -> bool {
    match combination {
        KeyCombination::CtrlUp | KeyCombination::CtrlDown => {
            log_debug(&format!("Handling Ctrl+{:?}", 
                if matches!(combination, KeyCombination::CtrlUp) { "Up" } else { "Down" }));
            ...
            if !input.is_empty() && app.display_container_state.focused_index.is_some() {
                match app.display_container_state.focused_index {
                    Some(0) | None => {
                        ...
                        crate::commands::execute_command(app, Some(crate::commands::Command::Complete(input)));
                    }
                    Some(idx) => {
                        if let Some(task_id) = app.display_container_state
                            .display_to_id
                            ...
                            .copied() 
                        {
                            ...
                            crate::commands::execute_command(app, Some(crate::commands::Command::Edit(task_id, input.clone())));
                            crate::commands::execute_command(app, Some(crate::commands::Command::CompleteById(task_id)));
                        }
                    }
                }
                return true;
